...
<body>
    ...
    @if(isset($challan))
        <script>
            ...
            window.challanId = {{ $challan->id }};
            window.editSaleData = @json($challan->load(['items'])->toArray());
    ...
    @elseif(isset($duplicateChallan))
        <script>
            ...
            window.challanId = null;
            window.editSaleData = @json(array_merge($duplicateChallan->load(['items'])->toArray(), ['bill_number' => $nextInvoiceNumber]));
            window.docType = 'delivery_challan';
        </script>
    @else
        <script>
